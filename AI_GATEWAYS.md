...
This document describes the **AI gateway benchmark** — a phase-by-phase latency, throughput, and reliability comparison of OpenRouter, Vercel AI Gateway, and Cloudflare AI Gateway, measured against a direct-to-Anthropic baseline.
...
- **Model**: Claude Haiku 4.5 for all four participants — `anthropic/claude-haiku-4.5` via OpenRouter's and Vercel AI Gateway's catalog alias, `claude-haiku-4-5-20251001` via Cloudflare's and Anthropic's own native model ID. Same underlying model, addressed the way each API expects it to be addressed.
...
- **`openai`** (OpenRouter, Vercel AI Gateway) — OpenAI-compatible `/chat/completions` shape, `Authorization: Bearer <key>`.
...
This is the single most important fairness property of this benchmark, worth stating plainly: **Cloudflare AI Gateway is called via its own direct-to-Anthropic passthrough route** (`/v1/{account}/{gateway}/anthropic/v1/messages`), not routed through OpenRouter or any other intermediary. OpenRouter and Vercel AI Gateway are each called via their own native routing to the same model. `anthropic-direct` calls Anthropic's API with no gateway at all, as the no-gateway control — it isolates how much latency each gateway adds on top of the underlying provider.
...
```
round 1: openrouter → vercel-ai-gateway → cloudflare-ai-gateway → anthropic-direct
round 2: openrouter → vercel-ai-gateway → cloudflare-ai-gateway → anthropic-direct
...
```
...
Required environment variables (`env.example`): `OPENROUTER_API_KEY`, `VERCEL_AI_GATEWAY_API_KEY`, `CLOUDFLARE_AI_GATEWAY_ACCOUNT_ID` + `CLOUDFLARE_AI_GATEWAY_GATEWAY_ID` (+ optional `CLOUDFLARE_AI_GATEWAY_TOKEN` if the gateway has Authenticated Gateway enabled), `ANTHROPIC_API_KEY` (shared by Cloudflare's passthrough and the direct baseline). Missing credentials cause that gateway to be reported as `SKIPPED` rather than failing the run.
...
- **Vantage-point dependent.** Results are a property of wherever the benchmark process runs — its network, region, and ISP — not a global ranking. The same benchmark run from a different location can produce a different ordering. We do not pin to any gateway's region or give any participant a network advantage; every gateway is called with the same code, from the same machine, in the same run.
